...
var func = {
    ...
    include: function(args,options){
        ...
        var entityFolders = app.get('entityFolders');
        /**
    ...
    angularBase: function(options,name,entity){
        var entityFolders = options.app.get('entityFolders');
        if(entityFolders) {
            /**
             * Create File
             */
        ...
        var file = fs.readFileSync(options.javaScriptPath).toString();
        var tpl =  fs.readFileSync(path.join(__dirname, '..', 'templates', '/'+entity+'.js')).toString();
        if(!new RegExp(name).test(file)){
            utils.write(options.javaScriptPath,file+'\n'+tpl.replace('{{name}}',entity=='directive' ? name.toLowerCase() : name));
        }
